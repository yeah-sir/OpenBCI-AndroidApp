...
public class SaveBytesToFile extends AsyncTask<byte[], Void, Void> {
	...
	protected Void doInBackground(byte[]... arduinoData) {
		...
		//Store packet data
		//The first byte is the Sample Number
		byte sampleNumber = packet[0];
		...
		channel[1] = OpenBCIDataConversion.convertByteToMicroVolts(packetDataChannel2);
		channel[2] = OpenBCIDataConversion.convertByteToMicroVolts(packetDataChannel3);
		channel[3] = OpenBCIDataConversion.convertByteToMicroVolts(packetDataChannel4);
		channel[4] = OpenBCIDataConversion.convertByteToMicroVolts(packetDataChannel5);
		channel[5] = OpenBCIDataConversion.convertByteToMicroVolts(packetDataChannel6);
